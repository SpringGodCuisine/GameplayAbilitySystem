...
UOverlayWidgetController* UAuraAbilitySystemLibrary::GetOverlayWidgetController(const UObject* WorldContextObject)
{
	...
	if (AAuraHUD* AuraHUD = nullptr; MakeWidgetControllerParams(WorldContextObject, WCParams, AuraHUD))
	{
...
void UAuraAbilitySystemLibrary::SetIsCriticalHit(FGameplayEffectContextHandle& EffectContextHandle, bool IsBlocked)
{
	if(FAuraGameplayEffectContext* AuraEffectContext = static_cast<FAuraGameplayEffectContext*>(EffectContextHandle.Get()))
	{
		AuraEffectContext->SetIsCriticalHit(IsBlocked);
	}
}
...
FGameplayEffectContextHandle UAuraAbilitySystemLibrary::ApplyDamageEffect(const FDamageEffectParams& DamageEffectParams)
{
	...
	//专门用来给 GameplayEffect 的 SetByCaller Magnitude 赋值 的蓝图/C++ 辅助函数
...
TArray<FRotator> UAuraAbilitySystemLibrary::EvenlySpacedRotators(const FVector& Forward, const FVector& Axis, float Spread, int32 NumRotators)
{
	...
	if (NumRotators > 1)
	{
		...
		{
			// RotateAngleAxis 是 UE 里 FVector 的一个旋转函数，用来让一个向量围绕某个轴旋转一个角度。
